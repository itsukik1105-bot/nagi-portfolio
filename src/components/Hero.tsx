...
export function Hero({ videoUrl, title, subtitle, onAboutClick }: HeroProps) {
  ...
  return (
    <section 
      // スマホ時は高さ60vh、PC以上で全画面
      className="relative h-[60vh] min-h-[500px] md:h-screen w-full overflow-hidden flex items-end justify-center pb-20 md:pb-40 bg-black"
    >
        ...
        {videoUrl ? (
          <div className="w-full h-full animate-slow-zoom">
            <video
              ...
              muted
              playsInline
              className="w-full h-full object-cover object-center filter contrast-[1.1] saturate-[0.9] brightness-[0.95]"
            >
      ...
      <div className="relative z-10 text-center px-4 fade-in-slow w-full">
        {/* タイトル */}
        <h1 
          className="text-4xl md:text-8xl font-bold tracking-tight text-white mb-6 md:mb-8"
        ...
        {/* サブタイトル */}
        <p className="text-[10px] md:text-sm text-white/70 tracking-[0.4em] uppercase mb-10 md:mb-12 font-medium border-t border-white/20 pt-6 md:pt-8 inline-block px-8">
          {subtitle}
        </p>
      ...
      {/* スクロールダウンライン */}
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-px h-16 md:h-24 bg-gradient-to-b from-white/0 via-white/40 to-white/0">
